...
public class ScreenCapture {
	...
	public static Uri  getBitmap(Activity activity, View layout){
		...
		bitmap = Bitmap.createBitmap(bitmap, 0,MathUtil.newInstance().dipToPx(60,activity),layout.getWidth(),
				layout.getHeight()-MathUtil.newInstance().dipToPx(60,activity));
		...
		if (file.exists()) {
			...
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
				...
				uri = FileProvider.getUriForFile(activity, "com.szip.blewatch.fileprovider", file);
			}
			return uri;
		}
		return null;
	...
	public static Uri  getScollerBitmap(Activity activity, ScrollView layout){
		...
		for (int i = 0; i < layout.getChildCount(); i++) {
			...
			layout.getChildAt(i).setBackgroundResource(R.color.bgColor);
		}
